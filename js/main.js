...
function getOptions(color, row, col, isKing){
    ...
    if(option.is(':parent')){
    if(option.children().first().css('background-color')
    !== color && !$('#'+(row+2)+'_'+(col-2)).is(':parent')){
...
function checkSkip(oldRow, oldCol, newRow, newCol){
    if(oldRow-newRow == 2){
    if(oldCol-newCol == -2){
...
getOptions(selected.css("background-color"),
...
$("#start").click(function(){
    ...
    if (i <= 2){ $("#"+id).append($("<div>",{class:"white piece"}));
    }
    else if 
    (i >= 5){
    ...
    $(".red.square").click(function() {
        ...
        if(color == "rgb(255, 255, 0)"){
            ...
            if(checkSkip( parseInt(pid[0]), parseInt(pid[1]), parseInt(id[0]), parseInt(id[1]))){
                if(whiteMove){
                    blackPieces--;
                    }else{
                    whitePieces--;
                    ...
                    if(parseInt(id[0]) ==0 || parseInt(id[0]) == 7 && !selected.is(':parent')){
